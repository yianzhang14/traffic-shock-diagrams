...
class Event:
    ...
    def __lt__(self, other: Event) -> bool:
        return (self.point.time < other.point.time) or (self.point.position < other.point.position)  # tiebreaker
...
class Interface:  # boundary between two states
    ...
    def __init__(self, point: dtPoint, slope: float, above: State=None, below: State=None, bounds: tuple[dtPoint, dtPoint]=[]):
        self.point = point
        self.slope = slope
    ...
    def get_pos_at_time(self, time: float) -> Optional[float]:
        if (self.endpoints[1] and self.endpoints[1].time < time) or (self.endpoints[0] and self.endpoints[0].time > time):
            return None
