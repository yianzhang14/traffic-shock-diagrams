...
class StateHandler:
    ...
    def __init__(self, simulation_time: float, max_dist: float, init_density: float, init_flow: float):
        ...
        self.states: list[ShapedState] = [ShapedState(State(init_density, init_flow), shp.Polygon(
            (
                (0, -self.max_dist),  # we allow max_dist to go negative to account for off-border interfaces (i think this is the limit of how low it can go)
