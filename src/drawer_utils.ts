...
export function floatIsClose(x: number, y: number): boolean {
...
export class dtPoint {
  ...
  public equalTo(other: dtPoint): boolean {
    return floatIsClose(this.time, other.time) && floatIsClose(this.position, other.position);
  };
  ...
  public getSlope(other: dtPoint): number {
    if (floatIsClose(this.time, other.time)) {
...
export class IntersectionEvent extends Event {
  ...
  constructor(point: dtPoint, interfaces: DiagramInterface[]) {
    ...
    for (const diagramInterface of interfaces) {
      if (diagramInterface instanceof UserInterface) {
...
export class CapacityEvent extends Event {
  public priorCapacity: number;
  public posteriorCapacity: number;
  public userInterface: UserInterface;

  /**
     * Creates an instance of CapacityEvent.
     * @param {dtPoint} point point at which the event takes place
     * @param {UserInterface} userInterface user interface that the event applies to
     * @param {number} [priorCapacity=-1] capacity before the event
     * @param {number} [posteriorCapacity=-1] capacity after the event (which is the key property of a CapacityEvent)
...
export class TruncationEvent extends Event {
  ...
  /**
     ...
     * @param {UserInterface} userInterface the user interface of the truncation event
     ...
     */
  constructor(point: dtPoint, userInterface: UserInterface, interfaces: DiagramInterface[]) {
...
export class State {
  ...
  public getInterfaceSlope(other: State): number {
    if (floatIsClose(this.density, other.density)) {
  ...
  public equalTo(other: State): boolean {
    return floatIsClose(this.density, other.density) && floatIsClose(this.flow, other.flow);
  }
...
export class DiagramInterface {
  ...
  public lowerBound: dtPoint;
  public upperBound: dtPoint;
  ...
  /**
     ...
     * @param {dtPoint} [lowerBound] lower bound of the interface; if not provided, is interpolated using the lower time bound & the provided point/slopoe
     * @param {dtPoint} [upperBound] upper bound of the interface; if not provided is set to Infinity, Infinity as a placeholder
  ...
  constructor(
    ...
    if (lowerBound === undefined) {
      this.lowerBound = new dtPoint(
        -1 * PlotThresholdOffset,
        point.position - slope * point.time - PlotThresholdOffset * slope
      );
    } else {
    ...
    if (upperBound === undefined) {
      this.upperBound = new dtPoint(Infinity, Infinity);
    } else {
  ...
  public hasEndpoint(point: dtPoint): boolean {
    if (this.lowerBound?.equalTo(point)) {
      return true;
    }

    if (this.upperBound?.equalTo(point)) {
      return true;
    }
  ...
  public getPosAtTime(time: number): number | undefined {
    ...
    if (floatIsClose(this.lowerBound.time, time)) {
      return this.lowerBound.position;
    }
    if (floatIsClose(this.upperBound.time, time)) {
      return this.upperBound.position;
    }

    // check if the two lines are completely disjoint -- return undefined
    if ((this.upperBound.time < time) || (this.lowerBound.time > time)) {
      return undefined;
    }
  ...
  public intersection(other: DiagramInterface): dtPoint | undefined {
    ...
    let midTime: number;
    if (this.upperBound.time === Infinity || other.upperBound.time == Infinity) {
      midTime = Math.max(this.lowerBound.time, other.lowerBound.time) + 1;
    } else {
      if (
        Math.abs(this.upperBound.time - other.lowerBound.time) <
        Math.abs(this.lowerBound.time - other.upperBound.time)
      ) {
        midTime = (this.upperBound.time + other.lowerBound.time) / 2;
      } else {
        midTime = (this.lowerBound.time + other.upperBound.time) / 2;
      }
    }

    // get position at that mid time
    let pos1: number | undefined = this.getPosAtTime(midTime);
    let pos2: number | undefined = other.getPosAtTime(midTime);
    ...
    if (
      floatIsClose(this.slope, other.slope)
      && pos1 !== undefined
      && pos2 !== undefined
      && floatIsClose(pos1, pos2)
    ) {
    ...
    } else if (floatIsClose(this.slope, other.slope)) {
      return undefined;
    }
  ...
  public addCutoff(lower?: dtPoint, upper?: dtPoint): void {
    ...
    if (lower !== undefined && !floatIsClose(this.slope, this.point.getSlope(lower))) {
      throw new RangeError("The lower bound supplied is invalid--it does not fall along the interface line");
    }

    if (upper !== undefined && !floatIsClose(this.slope, this.point.getSlope(upper))) {
    ...
    if (lower !== undefined) {
      if (this.lowerBound.time < lower.time) {
        this.lowerBound = lower;
      }
    }

    if (upper !== undefined) {
      if (this.upperBound.time > upper.time) {
  ...
  public equivalentTo(other: DiagramInterface): boolean {
    ...
    if (
      !floatIsClose(other.upperBound.time, this.upperBound.time)
      || !floatIsClose(other.lowerBound.time, this.lowerBound.time)
    ) {
      return false;
    }
    ...
    if (other.point.equalTo(this.point)) {
      return floatIsClose(other.slope, this.slope);
    }
    ...
    if (floatIsClose(other.point.time, this.point.time)) {
      return (
        floatIsClose(other.point.position, this.point.position)
        && floatIsClose(this.slope, other.slope)
      );
    }
    ...
    return (floatIsClose(this.point.getSlope(other.point), other.slope)
      && floatIsClose(other.slope, this.slope));
  }
  ...
  /**
     ...
     */
  public getSlope(): number {
    if (this.upperBound.time === Infinity) {
      throw new EvalError("Interface does not have well-defined endpoints");
    }

    return this.upperBound.getSlope(this.lowerBound);
  }
  ...
  public clone(): DiagramInterface {
    return new DiagramInterface(
      ...
      this.lowerBound ? this.lowerBound.clone() : undefined,
      this.upperBound ? this.upperBound.clone() : undefined
    );
  }
}
...
export class UserInterface extends DiagramInterface {
  ...
  public originalLowerBound: dtPoint;
  public originalUpperBound: dtPoint;
  ...
  /**
     ...
     * @param {dtPoint} lowerBound lower bound of the interface w.r.t. time
     * @param {dtPoint} upperBound upper bound of the interface w.r.t. time
  ...
  constructor(
    ...
    super(point, slope, undefined, undefined, lowerBound, upperBound);

    this.augment = augment;
    this.originalLowerBound = lowerBound.clone();
    this.originalUpperBound = upperBound.clone();
  }
  ...
  public clone(): UserInterface {
    ...
    temp.originalLowerBound = this.originalLowerBound;
    temp.originalUpperBound = this.originalUpperBound;
...
export class Trajectory extends DiagramInterface {
  constructor(point: dtPoint, slope: number, lowerBound?: dtPoint, upperBound?: dtPoint) {
    super(point, slope, undefined, undefined, lowerBound, upperBound);
  }
}
