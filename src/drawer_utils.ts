...
const PLOT_THRESHOLD_OFFSET: number = 1;
...
export class CapacityEvent extends Event {
     ...
     */
    constructor (point: dtPoint, user_interface: UserInterface, prior_capacity: number = -1, posterior_capacity: number = -1) {
...
export class DiagramInterface {
     ...
     */
    constructor(point: dtPoint, slope: number, above?: State, below?: State, lower_bound?: dtPoint, upper_bound?: dtPoint) {
        this.point = point;
        this.slope = slope;
        this.above = above;
        this.below = below;
        ...
        if (lower_bound === undefined) {
            this.lower_bound = new dtPoint(-1 * PLOT_THRESHOLD_OFFSET, point.position - slope * point.time - PLOT_THRESHOLD_OFFSET * slope);
        } else {
    ...
    public hasEndpoint(point: dtPoint): boolean {
        if (this.lower_bound !== undefined && this.lower_bound.equalTo(point)) {
            return true;
        }
        
        if (this.upper_bound !== undefined && this.upper_bound.equalTo(point)) {
            return true;
        }
    ...
    public intersection(other: DiagramInterface): dtPoint | undefined {
        ...
        if (this.upper_bound.time === Infinity || other.upper_bound.time == Infinity) {
            ...
            if (Math.abs(this.upper_bound.time - other.lower_bound.time) < Math.abs(this.lower_bound.time - other.upper_bound.time)) {
        ...
        if (float_isclose(this.slope, other.slope) && pos1 !== undefined && pos2 !== undefined && float_isclose(pos1, pos2)) {
        ...
        const timeOfIntersection: number = (other.point.position - other.slope * other.point.time - this.point.position + this.slope * this.point.time) / (this.slope - other.slope);
    ...
    public equivalentTo(other: DiagramInterface): boolean { 
        ...
        if ((other.below !== undefined && this.below !== undefined && !other.below.equalTo(this.below)) || (other.above !== undefined && this.above !== undefined && !other.above.equalTo(this.above))) {
            return false;
        }

        // if the two interfaces are disjoint in terms of endpoints, they are not equivalent
        if (!float_isclose(other.upper_bound.time, this.upper_bound.time) || !float_isclose(other.lower_bound.time, this.lower_bound.time)) {
            return false;
        }
        ...
        if (float_isclose(other.point.time, this.point.time)) {
            return float_isclose(other.point.position, this.point.position) && float_isclose(this.slope, other.slope);
        }
        ...
        return float_isclose(this.point.getSlope(other.point), other.slope) && float_isclose(other.slope, this.slope);
    }
...
export class UserInterface extends DiagramInterface {
     ...
     */
    constructor(point: dtPoint, slope: number, augment: CapacityBottleneck, lower_bound: dtPoint, upper_bound: dtPoint) {
    ...
    public clone(): UserInterface {
        let temp: UserInterface = super.clone() as UserInterface;
