...
from .diagram_utils import Event, EventType, State, dtPoint, Interface
from .state_handler import StateHandler

        
class ShockwaveDrawer:
    
    def __init__(self, diagram: FundamentalDiagram, simulation_time: float, augments: list[TrafficLight], init_density: float, init_flow: float):
        self.diagram = diagram
        self.simulation_time = simulation_time
        
        self.events: SortedList[Event] = SortedList()
        self.default_state = State(init_density, init_flow)
        # self.states = StateHandler(simulation_time, self.simulation_time * self.diagram.freeflow_speed, init_density, init_flow)
    ...
    def resolve_state(self, point: dtPoint, below: bool=True) -> State:
        # idea: on the dt-plane, you can get the interface that pertains to an event by looking directly down from the
        # event point (in the distance dimension) and taking the above state of the closest interface
    ...
    def run(self):
        while self.events:
            ...
            if cur.type == EventType.user:
                assert len(cur.interfaces) == 1  # if we assume augmenters always have an interface, this must be true
